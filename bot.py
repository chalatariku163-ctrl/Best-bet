...
def confidence(prediction):
    ...
    for key in ("home_percent", "draw_percent", "away_percent"):
        try:
            values.append(
                float(str(prediction.get(key, "0%")).replace("%", ""))
            )
        except Exception:
            pass
    ...
    if data == "home":
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    f"1️⃣ {odds['1']}",
                    callback_data=f"bet|{fixture_id}|1",
                ),
                InlineKeyboardButton(
                    f"❌ {odds['X']}",
                    callback_data=f"bet|{fixture_id}|X",
                ),
                InlineKeyboardButton(
                    f"2️⃣ {odds['2']}",
                    callback_data=f"bet|{fixture_id}|2",
                ),
            ],
            [
                InlineKeyboardButton(
                    f"⬆️ O2.5 {odds['Over 2.5']}",
                    callback_data=f"bet|{fixture_id}|O25",
                )
            ],
            [
                InlineKeyboardButton(
                    f"⬇️ U2.5 {odds['Under 2.5']}",
                    callback_data=f"bet|{fixture_id}|U25",
                )
            ],
            [
                InlineKeyboardButton(
                    f"⚽ BTTS {odds['BTTS']}",
                    callback_data=f"bet|{fixture_id}|BTTS",
                )
            ],
            [
                InlineKeyboardButton(
        ...
        if code not in mapping or not add_bet(
            user.id,
            selected,
            *mapping[code],
        ):
...
async function loadToday(){
    ...
    try{
        ...
        c.innerHTML =
            ...
            '</div>' +
            '<p class="status">' +
            'Check FOOTBALL_API_KEY on Render.' +
            '</p>';
    }
}
...
async function loadBest(){
    ...
    c.innerHTML =
        '<div class="loader">Analysing...</div>';
...
async function loadLive(){
    ...
    c.innerHTML =
        '<div class="loader">Loading live...</div>';
...
function renderSlip(){
    ...
    html += `
    <div class="panel">
        <b>Total Odds: ${total.toFixed(2)}</b>
    </div>
